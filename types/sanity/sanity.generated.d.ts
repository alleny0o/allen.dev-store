...
// Query: {  // announcement bar (intl, falls back to default language)  "announcementBar": coalesce(    announcementBar[_key == $language][0].value[],    announcementBar[_key == $defaultLanguage][0].value[],  )[] {    _type == "announcement" => {      _key,      externalLink,      link -> {  'documentType': _type,  'slug': coalesce(    slug,    store.slug  ) {    _type,    current  },},      openInNewTab,      text,    },  },  announcementBarColorScheme -> {  background {  alpha,  hex,  hsl,  rgb,},  border {  alpha,  hex,  hsl,  rgb,},  card {  alpha,  hex,  hsl,  rgb,},  cardForeground {  alpha,  hex,  hsl,  rgb,},  foreground {  alpha,  hex,  hsl,  rgb,},  primary {  alpha,  hex,  hsl,  rgb,},  primaryForeground {  alpha,  hex,  hsl,  rgb,},},  announcementBarPadding,  autoRotateAnnouncements,  // navigation menu (intl)  "menu": coalesce(    menu[_key == $language][0].value[],    menu[_key == $defaultLanguage][0].value[],  )[] {  _type == "externalLink" => {  _key,  _type,  link,  name,  openInNewTab,},  _type == "internalLink" => {  _key,  _type,  anchor,  link -> {  'documentType': _type,  'slug': coalesce(    slug,    store.slug  ) {    _type,    current  },},  name,},  _type == "nestedNavigation" => {  _key,  _type,  childLinks[] {    _type == "externalLink" => {  _key,  _type,  link,  name,  openInNewTab,},    _type == "internalLink" => {  _key,  _type,  anchor,  link -> {  'documentType': _type,  'slug': coalesce(    slug,    store.slug  ) {    _type,    current  },},  name,},  },  link -> {    'documentType': _type,    'slug': coalesce(      slug,      store.slug    ) {      _type,      current    },  },  name,},},  // desktop layout structure  layoutPreset,  desktopClusterGap,  // mobile layout structure  mobileLayoutPreset,  mobileClusterGap,  // visuals + behavior  colorScheme -> {  background {  alpha,  hex,  hsl,  rgb,},  border {  alpha,  hex,  hsl,  rgb,},  card {  alpha,  hex,  hsl,  rgb,},  cardForeground {  alpha,  hex,  hsl,  rgb,},  foreground {  alpha,  hex,  hsl,  rgb,},  primary {  alpha,  hex,  hsl,  rgb,},  primaryForeground {  alpha,  hex,  hsl,  rgb,},},  blur,  sticky,  showSeparatorLine,  padding,  desktopLogoWidth,}
export type HEADER_FRAGMENTResult = {
  ...
  menu: null;
  layoutPreset: never;
  desktopClusterGap: never;
  mobileLayoutPreset: never;
  mobileClusterGap: never;
  colorScheme: never;
  blur: never;
  sticky: never;
...
export type ROOT_QUERYResult = {
  ...
  header: {
    ...
    > | null;
    layoutPreset: 'layout-1' | 'layout-2' | 'layout-3' | 'layout-4' | null;
    desktopClusterGap: number | null;
    mobileLayoutPreset:
      | 'mobile-1'
      | 'mobile-2'
      | 'mobile-3'
